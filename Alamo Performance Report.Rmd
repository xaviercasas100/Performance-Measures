---
title: 'Alamo Performance Report BCY 23'
author:  
...
all_funds <- c("Title I Adult - Local","Title I DW - Local","WIOA Youth Local")
```
...
```{r Load Data}
setwd("C:/Users/casasxax/OneDrive - C2/Tableau Extracts/Alamo/Current")
...
MEQ2 <- MEQ2 %>% 
    ...
    mutate(Target = case_when(
        Program == "Title I Adult - Local" ~ 5500,
        Program == "Title I Youth - Local"~ 2700,
        Program == "Title I DW - Local"    ~ 7300,
        TRUE ~ 0),
...
EQ2 <- read_csv("EQ2.csv") %>% 
  ...
  mutate(Target=case_when(`Board ID` == 5 & Program == "Title I Adult - Local" ~ 72.50,
                          ...
                          `Board ID` == 18 & Program == "Title I DW - Local"~ 85.80,
...
EQ4 <- read_csv("EQ4.csv") %>% 
  ...
  mutate(Target=case_when(`Board ID` == 5 & Program == "Title I Adult - Local" ~ 76.50,
                          ...
                          `Board ID` == 20 & Program == "Title I DW - Local" ~ 85.00,
...
credentials <- credentials %>% 
    filter(Program %in% funds) %>% 
    mutate(local_wda_no = `Board Number`) %>% 
    distinct(`TWIST ID`,`Population Group Code`, .keep_all = TRUE) %>% 
    mutate(`Exit Date` = mdy_hm(`Exit Date`)) %>% 
    # filter( between(`Exit Date`,'2021-01-01','2021-12-31')) %>% 
  mutate(Quarter = case_when(`Exit Date` <= '2021-03-31' ~ "Q1",
                             `Exit Date` <= '2021-06-30' ~ "Q2",
                             `Exit Date` <= '2021-09-30' ~ "Q3",
                             TRUE ~ "Q4"),.after = `Exit Date`) %>% 
    mutate(Target = case_when(
        Program == funds[[1]] ~ 72.30,
        Program == funds[[2]] ~ 85.00)) %>% 
...
### Summary
```{r}
X_Summary_No_Youth(20)
X_Summary_All(20)
```

### Summary By Quarter
```{r}
X_Summary_Qtr_No_Youth(20)
```
...
```{r}
credentials %>% 
    # filter(`local_wda_no` %in% areas) %>%
    select(`TWIST ID`,`Exit Date`,Board,Program,`Texas Credential Numerator`) %>% 
