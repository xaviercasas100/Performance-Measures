---
title: 'Coastal Bend Report BCY 23'
author:  
...
source("H:/Performance Project/Functions/Functions.R")
```

```{r}
    # Fund and all funds are vectors we will use these to filter our extracts
funds <- c("Title I Adult - Local","Title I DW - Local")
all_funds <- c("Title I Adult - Local","Title I DW - Local","Title I Youth - Local")
    # areas is a paramater that we will use to run reports depending on what
    # wda area we want the information from.
```
...
```{r Load Data}
setwd("C:/Users/casasxax/OneDrive - C2/Tableau Extracts/Coastal Bend")
...
MEQ2 <- MEQ2 %>% 
    ...
    # Makes data unique by twist id and program
    distinct(`Twist Id`,Program,.keep_all = TRUE) %>% 
    # Sets the targets for median earnings depending on Program fund
    mutate(Target = case_when(
        Program == "Title I Adult - Local" & `LWDA Number` == 22 ~ 6600,
        Program == "Title I Youth - Local" & `LWDA Number` == 22 ~ 3200,
        Program == "Title I DW - Local" & `LWDA Number`    == 22 ~ 9400,
        TRUE ~ 0),
        # Converts the column to a date format
        `Exit Date` = mdy_hm(`Exit Date`)) %>% 
    # Bin data by exit date and create Qtr columns
    ...
    # Turns character columns into numeric
...
EEQ4 <- read_csv("EEQ4.csv") %>% 
    ...
    # Filter by the board parameter and keep the youth population
  filter(Program == "Title I Youth - Local") %>% 
    # Make distinct by twist id
  distinct(`Twist ID`, .keep_all = TRUE) %>% 
  mutate(`Exit Date`=mdy(`Exit Date`)) %>% 
    # Bin data into quarters
    ...
    # Set the target goals for each board
...
credentials <- credentials %>% 
  ...
  mutate(Quarter = case_when(`Exit Date` <= '2021-03-31' ~ "Q1",
    ...
    arrange(desc(`Texas Credential Numerator`)) %>% 
    slice(1L) %>% 
    ungroup()
```

```{r}
# Create tabs and use functions to create tables
```
...
```{r}
credentials %>% 
    filter(`Board Number` == 22) %>%
    select(`TWIST ID`,`Exit Date`, `Board Number`,Program,`Texas Credential Numerator`) %>% 
...
MSG %>% 
    filter(WDA_NO == 22) %>% 
    select(`Customer Id`, `MSG End Date`, `WDA Number`,Population_Desc, `MSG Numerator` ) %>% 
    mutate_all(as.factor) %>% 
    datatable(rownames = FALSE,
                caption = paste("Table"),
                filter = 'top',
