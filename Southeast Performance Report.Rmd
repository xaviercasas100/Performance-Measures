---
title: 'Southeast Performance Report BCY 23'
author:  
...
```{r Load Data}
setwd("C:\\Users\\casasxax\\OneDrive - C2\\Tableau Extracts\\Southeast")
...
MEQ2 <- MEQ2 %>% 
    ...
    mutate(Target = case_when(
        Program == "Title I Adult - Local" & `LWDA Number` == 18 ~ 7000,
        Program == "Title I Youth - Local" & `LWDA Number` == 18 ~ 3200,
        Program == "Title I DW - Local" & `LWDA Number`    == 18    ~ 9600,
        TRUE ~ 0),
...
EEQ4 <- read_csv("EEQ4.csv") %>% 
  ...
  distinct(`Participant Profile ID`, .keep_all = TRUE) %>% 
...
EQ2 <- read_csv("EQ2.csv") %>% 
  ...
  group_by(`Participant Profile ID`, Program) %>% 
    arrange(desc(Numerator)) %>% 
    ungroup() %>% 
    distinct(`Participant Profile ID`,Program,.keep_all = TRUE) %>% 
  mutate(`Exit Date`=mdy(`Exit Date`),
          Quarter=case_when(`Exit Date` <= '2021-09-30' ~ "Q1",
                           `Exit Date` <= '2021-12-31' ~ "Q2",
                           `Exit Date` <= '2022-03-31' ~ "Q3",
...
```
The rates used in this report are calculated using data extracts from Tableau/OKTA.
