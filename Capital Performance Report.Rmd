---
title: 'Capital Performance Report BCY 23'
author:  
...
```{r Load Data}
setwd("C:/Users/casasxax/OneDrive - C2/Tableau Extracts/Capital")
...
MSG <- MSG %>% 
    filter(POPULATION %in% c(4,5), WDA_NO == 14) %>% 
    mutate(BOARD = WDA_NO) %>% 
  mutate(`MSG_END_DT` = mdy(`MSG_END_DT`)) %>% 
...
### Summary
```{r}
X_Summary_No_Youth(14)
X_Summary_All(14)
```

### Summary By Quarter
```{r}
X_Summary_Qtr_No_Youth(14)
```
...
```{r}
MSG %>% 
    select(TWIST_ID, MSG_END_DT, WDA_NO,Population_Desc, WIOA_MSG_YTD_NUM ) %>% 
    mutate_all(as.factor) %>% 
    datatable(rownames = FALSE,
                caption = paste("Table"),
                filter = 'top',
